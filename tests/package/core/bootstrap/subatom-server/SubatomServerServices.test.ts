...
// =====================================================================
// 1. Mocks setup (Must be hoisted to the top)
// =====================================================================

// Hoist a reactive state object to control the mocked config module dynamically
const mockConfigState = vi.hoisted(() => ({
  exports: {} as any,
}));

// Intercept the exact dynamic import string used by the configLoader service
...
vi.mock("../../../../../package/core/pipeline/modifier/RouterPipelineAdapter.js", () => ({
...
vi.mock("../../../../../package/core/bootstrap/subatom-server/services/errorPipeline.service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../../../../package/core/bootstrap/subatom-server/services/errorPipeline.service.js")>();
  return {
    ...actual,
...
// =====================================================================
// Imports of the services to test
// =====================================================================
...
describe("SubatomServer Services", () => {
  ...
  describe("serverShutdown.service", () => {
    ...
    it("should force-destroy remaining sockets if the timeout is reached", () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("1 connection(s) still open after 5000ms; force-closing.")
      );
  ...
  describe("requestHandler.service", () => {
    it("should initialize request metadata and route through the pipeline", async () => {
      const nativeReq = {} as IncomingMessage;
    ...
    it("should catch pipeline errors and pass them to handleErrorPipeline", async () => {
      ...
      await processHttpRequest(nativeReq, nativeRes, {} as any, [], [], new AsyncLocalStorage(), {} as any);

      const { handleErrorPipeline: mockErrorPipeline } = await import("../../../../../package/core/bootstrap/subatom-server/services/errorPipeline.service.js");
      expect(mockErrorPipeline).toHaveBeenCalledWith(
        mockError, 
        expect.any(Object), 
        expect.any(Object), 
        []
  ...
  describe("portProber.service", () => {
    ...
    it("should retry the same port after 100ms if EADDRINUSE and retries left", async () => {
      ...
      const portPromise = getAvailablePort(8080, "localhost", 1);
    ...
    it("should reject immediately if an unknown error occurs", async () => {
      ...
      await expect(getAvailablePort(80, "localhost")).rejects.toEqual(unknownErr);
    });
  });
  ...
  describe("orphanRecovery.service", () => {
    ...
    it("should return true and route to error pipeline if response is still active", async () => {
      const mockReq = {};
      ...
      const { handleErrorPipeline: mockErrorPipeline } = await import("../../../../../package/core/bootstrap/subatom-server/services/errorPipeline.service.js");
      (mockErrorPipeline as Mock).mockClear();
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("Recovered an orphaned promise"));
      expect(mockErrorPipeline).toHaveBeenCalledWith(reason, mockReq, mockRes, []);
    });
  });
  ...
  describe("errorPipeline.service", () => {
    it("should bail early if response is already writableEnded", async () => {
      ...
      const { handleErrorPipeline: actualErrorPipeline } = await vi.importActual<any>("../../../../../package/core/bootstrap/subatom-server/services/errorPipeline.service.js");
    ...
    it("should iterate through error middlewares and fallback to ErrorFormatter", async () => {
      ...
      const mw1 = vi.fn(async (err, req, res, next) => {
        await next(new Error("Modified by mw1"));
      });
      const mw2 = vi.fn(async (err, req, res, next) => {
        await next(err); 
      });
    ...
    it("should catch exceptions thrown inside an error middleware and pass to next", async () => {
      ...
      const { handleErrorPipeline: actualErrorPipeline } = await vi.importActual<any>("../../../../../package/core/bootstrap/subatom-server/services/errorPipeline.service.js");
      await actualErrorPipeline(new Error(), {} as any, mockRes, [throwingMw as any]);
  ...
  describe("configLoader.service", () => {
    beforeEach(() => {
      // Reset the mocked export state before each test
      mockConfigState.exports = {};
    });

    it("should return empty object if import throws an error", async () => {
      // Alter process.cwd() so the dynamic import path misses our mock registry
      // and hits the real file system, intentionally causing it to throw ERR_MODULE_NOT_FOUND.
      const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue("/path/that/does/not/exist");
