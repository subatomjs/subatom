...
// 2. Mock Internal Modules using correct relative paths from tests/ to package/
...
// FIXED: Mock using a standard function so it behaves correctly as a constructor.
...
describe("SubatomServer", () => {
  ...
  beforeEach(() => {
    ...
    // Stub the HTTP Server instance to simulate event bindings and listening
    mockServerInstance = {
      on: vi.fn(),
    ...
    mockRouter = {} as Router; // Stub router
  });
  ...
  describe("Initialization & Constructor", () => {
    it("should initialize HTTP server and WebSocketManager", () => {
      ...
      // FIXED: Safely grab the newly instantiated mock context using mock.instances
  ...
  describe("Event Handling", () => {
    ...
    beforeEach(() => {
      ...
      // Extract the anonymous event handlers bound in the constructor
  ...
  describe("HTTP Request Handling", () => {
    it("should delegate native HTTP requests to processHttpRequest", async () => {
      ...
      const req = { on: vi.fn(), headers: {} } as unknown as IncomingMessage;
      const res = { on: vi.fn(), end: vi.fn() } as unknown as ServerResponse;
