...
import {
	mergeRouter,
	mergeSubRouter,
} from "../../../../package/core/router/services/routerMerger.service.js";
...
describe("Unit: routerMerger.service", () => {
	...
	it("should throw TypeError when target is not a valid Router instance", () => {
		const parent = new Router();
		expect(() =>
			mergeSubRouter(parent, "/api", {} as unknown as Router),
		).toThrow(TypeError);
		expect(() => mergeRouter(parent, null as unknown as Router)).toThrow(
			TypeError,
		);
	});
