...
describe("Unit: buildUrl", () => {
    ...
    it("should substitute single and multiple path parameters", () => {
        ...
        expect(buildUrl("/users/:id/posts/:postId", { id: 10, postId: 99 })).toBe("/users/10/posts/99");
    });

    it("should properly URI encode substituted parameter values", () => {
        expect(buildUrl("/search/:term", { term: "hello world&foo=bar" })).toBe("/search/hello%20world%26foo%3Dbar");
    });
    ...
    it("should throw an error when parameter value is null, undefined or empty string", () => {
        expect(() => buildUrl("/users/:id", { id: null as unknown as string })).toThrowError(
            '[Subatom] urlFor: missing required param ":id" for route "/users/:id".'
        );
        expect(() => buildUrl("/users/:id", { id: undefined as unknown as string })).toThrowError(
            '[Subatom] urlFor: missing required param ":id" for route "/users/:id".'
        );
    ...
    it("should throw an error when extraneous params not in the route path are supplied", () => {
        expect(() => buildUrl("/users/:id", { id: "123", extra: "val" })).toThrowError(
