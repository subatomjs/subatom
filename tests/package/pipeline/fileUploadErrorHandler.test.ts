...
describe("fileUploadErrorHandler", () => {
	...
	it("handles wrapped request raw.writeHead responses", () => {
		...
		fileUploadErrorHandler(
			new PayloadTooLargeError("Too big"),
			{} as any,
			res,
			next,
		);

		expect(res.raw.writeHead).toHaveBeenCalledWith(413, {
			"Content-Type": "application/json",
		});
		expect(res.raw.end).toHaveBeenCalledWith(
			JSON.stringify({ error: "Too big" }),
		);
	...
	it("handles Express-like res.status().json() responses", () => {
		...
		fileUploadErrorHandler(
			new UnprocessableEntityError("Invalid MIME"),
			{} as any,
			res,
			next,
		);
