...
import type { ISerializer, IPipelineContext } from "../../../package/types/framework/pipeline/IPipeline.js";
...
describe("runSerializers", () => {
  ...
  it("returns data unchanged when no serializer matches", async () => {
    ...
    const data = { raw: "data" };
    const result = await runSerializers([xmlSerializer], data, dummyCtx, "text/plain");
