...
describe("registerDocs - setupApiDocs", () => {
    ...
    it("should register custom docs path when provided in options", () => {
        ...
        expect(mockApp.get).toHaveBeenCalledWith("/openapi.json", expect.any(Function));
        expect(mockApp.get).toHaveBeenCalledWith("/api-documentation", expect.any(Function));
    });

    it("should respond with JSON OpenAPI spec on /openapi.json request", () => {
        const specSpy = vi.spyOn(openApiGen, "generateOpenApiSpec").mockReturnValue({
            openapi: "3.0.0",
    ...
    it("should pass only defined options to generateOpenApiSpec", () => {
        const specSpy = vi.spyOn(openApiGen, "generateOpenApiSpec").mockReturnValue({} as any);
    ...
    it("should serve rendered Swagger UI HTML on documentation route", () => {
        ...
        expect(mockRes.send).toHaveBeenCalledWith("<html><body>Swagger</body></html>");
