...
describe("handleUpgrade Service", () => {
    ...
    it("should destroy socket if URL is unparseable", async () => {
        ...
        await handleUpgrade(req, mockSocket, Buffer.alloc(0), mockWss, routes, {}, vi.fn());
    ...
    it("should respond with 404 and end socket if route is not registered", async () => {
        const { mockSocket, mockWss, routes } = createMocks();
        const req = { url: "/unregistered", headers: { host: "localhost" } } as IncomingMessage;
    ...
    it("should respond with 401 and end socket if verifyClient returns false", async () => {
        ...
        const req = { url: "/ws", headers: { host: "localhost" } } as IncomingMessage;
    ...
    it("should respond with 401 if verifyClient throws an exception", async () => {
        ...
        const options = {
            verifyClient: vi.fn().mockRejectedValue(new Error("JWT Verification failed")),
        };
        const req = { url: "/secure", headers: { host: "localhost" } } as IncomingMessage;

        await handleUpgrade(req, mockSocket, Buffer.alloc(0), mockWss, routes, options, vi.fn());
    ...
    it("should complete upgrade and invoke onConnectionEstablished on valid request", async () => {
        ...
        await handleUpgrade(req, mockSocket, head, mockWss, routes, {}, onEstablishedMock);

        expect(mockWss.handleUpgrade).toHaveBeenCalledWith(req, mockSocket, head, expect.any(Function));
        expect(onEstablishedMock).toHaveBeenCalledWith("rawSocketInstance", req, route);
    });
});