...
describe("compressor.ts - SubatomCompression", () => {
    ...
    describe("Constructor and Options Normalization", () => {
        it("initializes with robust defaults", () => {
            ...
            expect(engine.opts.sensitiveHeaders).toEqual(["set-cookie", "x-csrf-token"]);
            expect(engine.opts.levels.br).toBe(4);
            expect(engine.opts.levels.gzip).toBe(zlib.constants.Z_DEFAULT_COMPRESSION);
            expect(engine.opts.levels.deflate).toBe(zlib.constants.Z_DEFAULT_COMPRESSION);
        });
        ...
        it("normalizes custom options and clamps levels properly", () => {
            ...
            expect(engine.opts.sensitiveHeaders).toEqual(["x-auth-token", "authorization"]);
