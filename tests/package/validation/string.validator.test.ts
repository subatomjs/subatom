...
describe("compileStringValidator", () => {
	...
	it("should validate built-in formats (email, url, uri, uuid, date, ipv4)", async () => {
		const emailValidator = compileStringValidator({
			type: "string",
			format: "email",
		});
		...
		const urlValidator = compileStringValidator({
			type: "string",
			format: "url",
		});
		expect(await urlValidator("https://subatom.dev/docs", "url")).toHaveLength(
			0,
		);
		expect(await urlValidator("ftp://files.example.com", "url")).toHaveLength(
			0,
		);
		...
		const uriValidator = compileStringValidator({
			type: "string",
			format: "uri",
		});
		...
		const uuidValidator = compileStringValidator({
			type: "string",
			format: "uuid",
		});
		expect(
			await uuidValidator("123e4567-e89b-12d3-a456-426614174000", "id"),
		).toHaveLength(0);
		expect(await uuidValidator("invalid-uuid", "id")).toEqual([
			{ path: "id", rule: "format", message: "Invalid uuid format" },
		]);

		const dateValidator = compileStringValidator({
			type: "string",
			format: "date",
		});
		...
		const ipv4Validator = compileStringValidator({
			type: "string",
			format: "ipv4",
		});
