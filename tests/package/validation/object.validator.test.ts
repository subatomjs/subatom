...
describe("compileObjectValidator", () => {
    it("should reject non-object and array types", async () => {
        ...
        expect(await validator(null, "user")).toEqual([
            { path: "user", rule: "type", message: "Expected object", received: "object" },
        ]);
        expect(await validator([], "user")).toEqual([
            { path: "user", rule: "type", message: "Expected object", received: "object" },
        ]);
        expect(await validator("string", "user")).toEqual([
            { path: "user", rule: "type", message: "Expected object", received: "string" },
        ]);
    });
    ...
    it("should enforce required fields", async () => {
        ...
        expect(await validator({ id: 1, email: "a@b.com" }, "data")).toHaveLength(0);
        ...
        expect(nullRequiredRes).toEqual([
            { path: "id", rule: "required", message: "Missing required property: id" },
            { path: "email", rule: "required", message: "Missing required property: email" },
        ]);
    });
    ...
    it("should validate nested properties", async () => {
        ...
        expect(await validator({ user: { name: "Bob" } }, "payload")).toHaveLength(0);
