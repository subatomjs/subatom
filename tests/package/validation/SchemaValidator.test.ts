...
describe("SchemaValidator", () => {
    ...
    it("should handle nullable schemas correctly", async () => {
        const nonNullableValidator = SchemaValidator.compile({ type: "string", nullable: false });
        expect(await nonNullableValidator(null, "field")).toEqual([
            { path: "field", rule: "nullable", message: "Value cannot be null or undefined" },
        ]);
        expect(await nonNullableValidator(undefined, "field")).toEqual([
            { path: "field", rule: "nullable", message: "Value cannot be null or undefined" },
        ]);

        const nullableValidator = SchemaValidator.compile({ type: "string", nullable: true });
