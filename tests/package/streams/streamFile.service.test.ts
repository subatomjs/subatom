...
describe("streamFile.service", () => {
	...
	describe("parseRange", () => {
		...
		it("should parse standard start-end ranges", () => {
			expect(parseRange("bytes=0-499", FILE_SIZE)).toEqual({
				start: 0,
				end: 499,
			});
			expect(parseRange("bytes=500-999", FILE_SIZE)).toEqual({
				start: 500,
				end: 999,
			});
		});
		...
		it("should parse open-ended start- ranges", () => {
			expect(parseRange("bytes=500-", FILE_SIZE)).toEqual({
				start: 500,
				end: 999,
			});
		});
		...
		it("should parse suffix ranges (-suffixLength)", () => {
			expect(parseRange("bytes=-200", FILE_SIZE)).toEqual({
				start: 800,
				end: 999,
			});
		});
		...
		it("should cap suffix range start at 0 if suffix exceeds file size", () => {
			expect(parseRange("bytes=-1500", FILE_SIZE)).toEqual({
				start: 0,
				end: 999,
			});
		});
		...
		it("should cap end at size - 1 if end exceeds file size", () => {
			expect(parseRange("bytes=500-2000", FILE_SIZE)).toEqual({
				start: 500,
				end: 999,
			});
		});
		...
		it("should throw RangeNotSatisfiableError for invalid ranges", () => {
			expect(() => parseRange("bytes=500-200", FILE_SIZE)).toThrow(
				RangeNotSatisfiableError,
			);
			expect(() => parseRange("bytes=-0", FILE_SIZE)).toThrow(
				RangeNotSatisfiableError,
			);
			expect(() => parseRange("bytes=abc-def", FILE_SIZE)).toThrow(
				RangeNotSatisfiableError,
			);
			expect(() => parseRange("bytes=-abc", FILE_SIZE)).toThrow(
				RangeNotSatisfiableError,
			);
			expect(() => parseRange("bytes=2000-3000", FILE_SIZE)).toThrow(
				RangeNotSatisfiableError,
			);
		});
	});
