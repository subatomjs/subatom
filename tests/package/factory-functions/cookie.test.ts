...
import { parseCookieHeader, serializeCookie } from "../../../package/core/factory-functions/utils/cookie.js"
...
describe("Cookie Utility (parseCookieHeader & serializeCookie)", () => {
    describe("parseCookieHeader", () => {
        ...
        it("should ignore malformed cookie segments without '=' or empty keys", () => {
            const header = "valid=yes; ; malformedWithoutEquals; =emptyKey; another=1";
            expect(parseCookieHeader(header)).toEqual({
                valid: "yes",
                another: "1",
            });
    ...
    describe("serializeCookie", () => {
        ...
        it("should properly URI encode cookie values", () => {
            const cookie = serializeCookie("session data", "val with spaces & special=chars");
            expect(cookie).toBe("session data=val%20with%20spaces%20%26%20special%3Dchars; Path=/");
        });
        ...
        it("should handle sameSite string options ('lax', 'strict', 'none') with capitalization", () => {
            expect(serializeCookie("sid", "123", { sameSite: "lax" })).toContain("; SameSite=Lax");
            expect(serializeCookie("sid", "123", { sameSite: "strict" })).toContain("; SameSite=Strict");
            expect(serializeCookie("sid", "123", { sameSite: "none" })).toContain("; SameSite=None");
        });
    });
});