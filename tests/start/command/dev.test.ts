...
import { ProcessManager } from "../../../start/watch/index.js";
...
vi.mock("../../../start/watch/index.js", () => {
	return {
		...
		FrameworkWatcher: vi.fn().mockImplementation(function (
			this: any,
			opts: any,
		) {
			this.options = opts;
...
describe("runDev", () => {
	...
	it("should exit 1 if port is NaN", async () => {
		await expect(runDev({ port: "invalid-port" })).rejects.toThrow(
			"process.exit called",
		);
	...
	it("should handle watcher onChange (single file and batched files) and onError", async () => {
		...
		expect(managerInstance.restart).toHaveBeenCalledWith(
			"2 files (src/index.ts and others)",
		);

		// Error callback
		watcherOptions.onError(new Error("Watcher failed"));
		expect(loggerErrorSpy).toHaveBeenCalledWith(
			"Watcher error: Watcher failed",
		);
	});
