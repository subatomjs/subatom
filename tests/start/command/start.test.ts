...
describe("runStart", () => {
  ...
  it("should exit 1 when an invalid port range or non-integer is passed", async () => {
    ...
    await expect(runStart({ port: "-1" })).rejects.toThrow("process.exit called");
    await expect(runStart({ port: "abc" })).rejects.toThrow("process.exit called");
  });
  ...
  it("should format host 0.0.0.0, ::, and ::0 to localhost", async () => {
    ...
    expect(loggerSuccessSpy).toHaveBeenCalledWith("Starting production server on http://localhost:3000");
  ...
  it("should format IPv6 with brackets only when not already bracketed", async () => {
    ...
    expect(loggerSuccessSpy).toHaveBeenCalledWith("Starting production server on http://[::1]:9000");
