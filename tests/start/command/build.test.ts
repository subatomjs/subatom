...
describe("runBuild", () => {
  ...
  it("should format plural source and singular static logs", async () => {
    vi.spyOn(fs, "existsSync").mockReturnValue(true);
    vi.spyOn(fs, "readdirSync").mockImplementation(() => ["a.ts", "b.ts", "data.json"] as any);
    ...
    expect(loggerSuccessSpy).toHaveBeenCalledWith("Compiled 2 source files successfully.");
    expect(loggerSuccessSpy).toHaveBeenCalledWith("Copied 1 asset successfully.");
  });
  ...
  it("should log singular source and static asset counts", async () => {
    vi.spyOn(fs, "existsSync").mockReturnValue(true);
    vi.spyOn(fs, "readdirSync").mockReturnValue(["index.ts", "data.json"] as any);
    vi.spyOn(fs, "statSync").mockReturnValue({ isDirectory: () => false } as any);
    vi.spyOn(fs, "mkdirSync").mockReturnValue(undefined as any);
    vi.spyOn(fs, "copyFileSync").mockReturnValue(undefined);
    vi.spyOn(fs, "renameSync").mockReturnValue(undefined);
