...
describe("ProcessManager", () => {
	...
	let _loggerInfoSpy: ReturnType<typeof vi.spyOn>;

	beforeEach(() => {
		vi.clearAllMocks();
		loggerErrorSpy = vi.spyOn(logger, "error").mockImplementation(() => {});
		_loggerInfoSpy = vi.spyOn(logger, "info").mockImplementation(() => {});
	...
	it("should log errors when process exits with a non-zero code", () => {
		...
		expect(loggerErrorSpy).toHaveBeenCalledWith(
			"app-server crashed (exit code 1)",
		);
		expect(pm.getStatus().isRunning).toBe(false);
	});
	...
	it("should log errors on spawn error event", () => {
		...
		expect(loggerErrorSpy).toHaveBeenCalledWith(
			"Failed to start app-server: EACCES",
		);
		expect(pm.getStatus().isRunning).toBe(false);
	});
