...
describe("openApiGenerator", () => {
  ...
  describe("convertInferSchema", () => {
    ...
    it("should handle array and object schemas with unsupported nested values", () => {
      ...
      expect(
        convertInferSchema(arrayWithUnsupportedItems),
      ).toEqual({ type: "array" });
      ...
      expect(
        convertInferSchema({ _type: "object", shape: "invalid" }),
      ).toEqual({ type: "object" });
  ...
  describe("generateOpenApiSpec", () => {
    ...
    it("should iterate candidate handlers covering all loop branches (empty, non-match, and match)", () => {
      ...
      const spec = generateOpenApiSpec([matchingRoute, nonMatchingRoute, emptyRoute]);
      expect(spec.paths["/match-handlers"].post.requestBody?.content["multipart/form-data"]).toBeDefined();
    ...
    it("should cover path parameter optionality branches at line 451", () => {
      ...
      const spec = generateOpenApiSpec(routes);
      const params = spec.paths["/org/{orgId}/users/{userId}/{role}"].get.parameters ?? [];
    ...
    it("should reuse a component reference for the same schema object", () => {
      ...
      expect(
        spec.paths["/shared-two"].post.requestBody?.content[
          "application/json"
        ].schema,
      ).toEqual(
        spec.paths["/shared-one"].post.requestBody?.content[
          "application/json"
        ].schema,
      );
    });
