...
describe("Body Parsing Middlewares", () => {
	...
	describe("raw()", () => {
		it("should skip and assign empty Buffer if content-type does not match", async () => {
			const { req, res } = createMockRequestResponse({
				headers: {
		...
		it("should parse binary data to Buffer when matching type array or string", async () => {
	...
	describe("text()", () => {
		it("should skip and set req.body = '' when type does not match", async () => {
			const { req, res } = createMockRequestResponse({
		...
		it("should parse text payload using specified encoding", async () => {
			const { req, res } = createMockRequestResponse({
				headers: {
	...
	describe("xml() & parseNativeXml", () => {
		it("should skip parsing when content-type is not XML", async () => {
			const { req, res } = createMockRequestResponse({
				headers: { "content-type": "text/plain" },
			});
			const next = vi.fn();
		...
		it("should parse complex XML including attributes, comments, text and entities", async () => {
			const xmlPayload = `
				<?xml version="1.0" encoding="UTF-8"?>
				<!-- Configuration file comment -->
				<catalog status="active" version='2'>
					<title>Subatom &amp; Node</title>
		...
		it("should return 400 when XML contains unmatched closing tags or unclosed CDATA", async () => {
			const { req, res, resHelper } = createMockRequestResponse({
				headers: {
					"content-type": "application/xml",
					"content-length": "30",
				},
				bodyPayload: "<root><open></mismatch></root>",
			});
		...
		it("should return 413 when an XML Buffer exceeds the configured limit", async () => {
			// Arrange
		...
		it("should return the XML fallback response for an invalid content length", async () => {
			// Arrange
			const { req, res, resHelper } = createMockRequestResponse({
				headers: {
					"content-type": "application/xml",
