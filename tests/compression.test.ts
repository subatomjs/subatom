...
const LARGE_TEXT = "Subatom compression test ".repeat(5000);
...
async function rawRequest(
	...
	if (!address || typeof address === "string") {
		throw new Error("Test server does not have a TCP address");
	}
...
async function decompressResponse(
	...
	switch (algorithm) {
		case "br":
			return new Promise((resolve, reject) => {
				zlib.brotliDecompress(body, (error, result) => {
					if (error) {
						reject(error);
						return;
					}
...
function contentLength(response: RawResponse): number | undefined {
...
function createTestServer(options: CompressionOptions = {}): Server {
	const compression = createCompression(options);
		...
		compression(req, res, () => {
			switch (req.url) {
				...
				case "/small":
					...
					res.setHeader("Content-Length", Buffer.byteLength(SMALL_TEXT));
				...
				case "/html":
					...
					res.end(`<html><body>${LARGE_TEXT}</body></html>`);
					return;
				...
				case "/cookie":
					...
					res.end(
						JSON.stringify({
							secret: "very-secret".repeat(1000),
						}),
					);
					return;
				...
				case "/csrf":
					...
					res.setHeader("X-CSRF-Token", "super-secret-token");
				...
				case "/head":
					...
					res.setHeader("Content-Length", Buffer.byteLength(LARGE_JSON));
				...
				case "/stream":
					...
					res.write("chunk-one ".repeat(500));

					setTimeout(() => {
						res.write("chunk-two ".repeat(500));

						setTimeout(() => {
							res.end("chunk-three ".repeat(500));
						}, 5);
					}, 5);
...
describe("SubatomCompression - negotiation", () => {
	let compression: SubatomCompression;

	beforeEach(() => {
		compression = new SubatomCompression();
	});

	it("uses server preference when Accept-Encoding is absent", () => {
		const result = compression.negotiate(undefined);
	...
	it("uses identity for an explicitly empty Accept-Encoding header", () => {
		const result = compression.negotiate("");
	...
	it("prefers Brotli when all algorithms have equal q-values", () => {
		const result = compression.negotiate("gzip, deflate, br");

		expect(result.algorithm).toBe("br");
	});

	it("prefers the highest client q-value", () => {
		const result = compression.negotiate("gzip;q=1.0, br;q=0.5");
	...
	it("uses server preference when q-values tie", () => {
		const result = compression.negotiate("gzip;q=0.8, br;q=0.8");

		expect(result.algorithm).toBe("br");
	});

	it("never selects an explicitly forbidden encoding", () => {
		const result = compression.negotiate("br;q=0, gzip;q=0.8");

		expect(result.algorithm).toBe("gzip");
	});

	it("does not allow wildcard to override explicit q=0", () => {
		const result = compression.negotiate("br;q=0, *;q=1");

		expect(result.algorithm).not.toBe("br");
	});

	it("applies wildcard to unspecified algorithms", () => {
		const result = compression.negotiate("*;q=0.5");

		expect(result.algorithm).toBe("br");

		expect(result.qValue).toBe(0.5);
	});

	it("respects explicit gzip over wildcard", () => {
		const result = compression.negotiate("gzip;q=0.9, *;q=0.2");

		expect(result.algorithm).toBe("gzip");

		expect(result.qValue).toBe(0.9);
	});

	it("handles identity;q=0 correctly", () => {
		const result = compression.negotiate("br;q=0, gzip;q=0, identity;q=0");

		expect(result.acceptable).toBe(false);
	});

	it("allows compression when identity is forbidden", () => {
		const result = compression.negotiate("br;q=1, identity;q=0");
	...
	it("handles wildcard q=0", () => {
		const result = compression.negotiate("*;q=0");
	...
	it("allows explicit identity to override wildcard q=0", () => {
		const result = compression.negotiate("*;q=0, identity;q=1");
	...
	it("is case insensitive", () => {
		const result = compression.negotiate("GZIP");

		expect(result.algorithm).toBe("gzip");
	});

	it("accepts three decimal q-values", () => {
		const result = compression.negotiate("gzip;q=0.800, br;q=0.500");
	...
	it("rejects malformed q-values", () => {
		const result = compression.negotiate("gzip;q=abc, br;q=0.8");

		expect(result.algorithm).toBe("br");
	});

	it("rejects q-values above 1", () => {
		const result = compression.negotiate("gzip;q=1.1, br;q=0.8");

		expect(result.algorithm).toBe("br");
	});

	it("rejects q-values with more than three decimal digits", () => {
		const result = compression.negotiate("gzip;q=0.1234, br;q=0.8");

		expect(result.algorithm).toBe("br");
	});

	it("supports custom server algorithm order", () => {
		const custom = new SubatomCompression({
			algorithms: ["gzip", "br", "deflate"],
		});

		const result = custom.negotiate("gzip, br");
	...
	it("falls back to identity when configured algorithms are unsupported", () => {
		...
		const result = custom.negotiate("br;q=1");
...
describe("SubatomCompression - isCompressible", () => {
	let compression: SubatomCompression;
	...
	it("rejects responses with CSRF headers", () => {
		...
		res.setHeader("X-CSRF-Token", "secret");

		expect(compression.isCompressible(req, res)).toBe(false);
	});

	it("allows sensitive responses when mitigation is disabled", () => {
		const noMitigation = new SubatomCompression({
		...
		expect(noMitigation.isCompressible(req, res)).toBe(true);
	});
	...
	it("supports custom MIME types", () => {
		...
		expect(custom.isCompressible(req, res)).toBe(true);
	});

	it("supports custom shouldCompress predicate", () => {
		const custom = new SubatomCompression({
			shouldCompress: (request) => request.headers["x-no-compress"] !== "true",
		});
		...
		expect(custom.isCompressible(req, res)).toBe(false);
	});
...
describe("SubatomCompression - HTTP integration", () => {
	...
	it("compresses large JSON with Brotli", async () => {
		...
		const decompressed = await decompressResponse("br", response.body);

		expect(decompressed.toString()).toBe(LARGE_JSON);
	});
	...
	it("compresses large JSON with gzip", async () => {
		...
		const decompressed = await decompressResponse("gzip", response.body);
	...
	it("compresses with deflate when explicitly requested", async () => {
		...
		const decompressed = await decompressResponse("deflate", response.body);
	...
	it("does not compress small responses below threshold", async () => {
		...
		expect(response.body.toString()).toBe(SMALL_TEXT);
	});
...
describe("SubatomCompression - Accept-Encoding HTTP semantics", () => {
	let server: Server | undefined;
	...
	it("does not compress when client explicitly requests identity only", async () => {
		...
		expect(response.body.toString()).toBe(LARGE_JSON);
	});
	...
	it("returns 406 when every representation is forbidden", async () => {
		...
		const response = await rawRequest(server, {
			path: "/json",
			headers: {
				"Accept-Encoding": "br;q=0, gzip;q=0, deflate;q=0, identity;q=0",
			},
		});

		expect(response.statusCode).toBe(406);

		expect(response.body.toString()).toBe("Not Acceptable");
	});
...
describe("SubatomCompression - Vary", () => {
	let server: Server | undefined;
	...
	it("does not duplicate Accept-Encoding", async () => {
		...
		expect(vary).toBe("user-agent, accept-encoding");
	});
...
describe("SubatomCompression - streaming", () => {
	let server: Server | undefined;
	...
	it("preserves write overloads", async () => {
		...
		expect(decompressed.toString()).toBe("firstsecondthird");
	});
...
describe("SubatomCompression - HTTP special responses", () => {
	let server: Server | undefined;
...
describe("SubatomCompression - compression integrity", () => {
	let server: Server | undefined;
	...
	it.each(["br", "gzip", "deflate"] as const)(
		...
		async (algorithm) => {
			...
			expect(response.headers["content-encoding"]).toBe(algorithm);

			const decompressed = await decompressResponse(algorithm, response.body);
	...
	it("compressed response is smaller than original for highly compressible data", async () => {
		...
		expect(response.body.length).toBeLessThan(Buffer.byteLength(LARGE_JSON));
	});
});
...
describe("SubatomCompression - configuration", () => {
	let server: Server | undefined;
...
describe("SubatomCompression - compressor streams", () => {
	let compression: SubatomCompression;
	...
	it("creates Brotli compressor", () => {
		const stream = compression.createCompressorStream("br");
	...
	it("creates Gzip compressor", () => {
		const stream = compression.createCompressorStream("gzip");
	...
	it("creates Deflate compressor", () => {
		const stream = compression.createCompressorStream("deflate");
	...
	it("returns null for identity", () => {
		const stream = compression.createCompressorStream("identity");
