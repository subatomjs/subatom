...
import {
	...
	registerGroupRoute,
} from "../../../../../packages/core/router/services/routeRegistrar.service.js";
...
import {
	extractPathname,
	extractQuery,
} from "../../../../../packages/core/router/services/urlParser.service.js";
import {
	mergeRouter,
	mergeSubRouter,
} from "../../../../../packages/core/router/services/routerMerger.service.js";
...
describe("router service modules", () => {
	...
	describe("route matching", () => {
		...
		it("should use GET defaults, skip middleware, and match ALL routes", () => {
			...
			const defaultResult = matchRoute(
				[middleware, makeRoute({ path: "/users/:id" })],
				undefined,
				"/users/1",
			);
	...
	describe("route registry", () => {
		...
		it("should generate a name and reject invalid methods, handlers, and handler entries", () => {
			...
			expect(() => registerWithMeta(routes, "", "/", [handler])).toThrow(
				TypeError,
			);
			expect(() => registerWithMeta(routes, "GET", "/", [])).toThrow(TypeError);
			expect(() =>
				registerWithMeta(routes, "GET", "/", [
					"not a handler" as unknown as IHandler,
				]),
			).toThrow(TypeError);
		...
		it("should use the default path in registry validation errors", () => {
			...
			expect(() => registerWithMeta(routes, "GET", "", [])).toThrow(
				/route "GET \/"/,
			);
			expect(() =>
	...
	describe("route registration", () => {
		it("should register a grouped options route with merged metadata", () => {
			...
			const router = {
				registerWithMeta: registerWithMetaMock,
			...
			registerPossiblyGrouped(router, context, "GET", "/users", [
				{ controller, tags: ["route", "group"] },
			]);
		...
		it("should register ungrouped options and ordinary handler routes", () => {
			...
			registerPossiblyGrouped(router, undefined, "GET", "/users", [
				{ controller },
			]);
			registerPossiblyGrouped(router, undefined, "POST", "/users", [
				handler,
				{ name: "users" },
			]);
		...
		it("should reject invalid route paths and missing handlers", () => {
			...
			expect(() =>
				registerPossiblyGrouped(router, undefined, "GET", "", [handler]),
			).toThrow(TypeError);
			expect(() =>
				registerPossiblyGrouped(router, undefined, "GET", "/users", []),
			).toThrow(TypeError);
	...
	describe("router merging", () => {
		...
		it("should reject invalid routers and duplicate names, and merge unprefixed routes", () => {
			...
			const validSource = {
				getRoutes: vi.fn(() => [makeRoute({ name: undefined })]),
			} as unknown as IRouter;

			expect(() =>
				mergeSubRouter(target, "/api", {} as unknown as IRouter),
			).toThrow(TypeError);
			expect(() => mergeSubRouter(target, "/api", source)).toThrow(TypeError);
			mergeRouter(target, validSource);
			expect(() => mergeRouter(target, {} as unknown as IRouter)).toThrow(
				TypeError,
			);
		});
		...
		it("should register group routes without context options", () => {
			...
			registerPossiblyGrouped(mockRouter, undefined, "GET", "/plain", [
				vi.fn() as unknown as RouteArgument,
			]);
		...
		it("should cover all optional metadata branches in registerPossiblyGrouped", () => {
			...
			registerPossiblyGrouped(router, context, "GET", "/no-tags", [
				{ controller: vi.fn() },
			]);
			expect(registerWithMetaMock).toHaveBeenCalledWith(
				"GET",
		...
		it("should merge context tags when options tags are omitted in registerPossiblyGrouped", () => {
			...
			registerPossiblyGrouped(router, context, "GET", "/settings", [
				{ controller: vi.fn() },
			]);
			expect(registerWithMetaMock).toHaveBeenCalledWith(
				"GET",
		...
		it("should merge sub-router routes with undefined optional fields", () => {
			...
			const source = {
				getRoutes: vi.fn(() => [minimalistRoute]),
			} as unknown as IRouter;
