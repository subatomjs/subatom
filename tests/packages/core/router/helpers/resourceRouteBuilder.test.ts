...
describe("buildResourceRoutes", () => {
	...
	it("should reject invalid resource definitions and unknown only actions", () => {
		expect(() => buildResourceRoutes("", dummyController)).toThrowError(
			/basePath/,
		);
		expect(() =>
			buildResourceRoutes("posts", null as unknown as IResourceController),
		).toThrowError(/controller object/);
		expect(() =>
			buildResourceRoutes("posts", dummyController, {
				only: ["destroy", "index"],
			}),
		...
		expect(() =>
			buildResourceRoutes("profile", dummyController, {
				only: ["invalid" as never],
			}),
	...
	it("should skip missing actions and preserve explicit patch handlers", () => {
		...
		const routes = buildResourceRoutes("posts", controller, {
			allowPatch: false,
		});
	...
	it("should auto-serialize objects via ctx.json and skip serialization when response already ended", async () => {
		...
		const indexHandler = routes.find(
			(r) => r.method === "GET" && r.path === "/api/json",
		)?.handlers[0];
		if (indexHandler) await indexHandler(dummyReq, dummyRes, next);
		expect(
			(dummyRes as unknown as { json: ReturnType<typeof vi.fn> }).json,
		).toHaveBeenCalledWith({ success: true });

		const showHandler = routes.find(
			(r) => r.method === "GET" && r.path === "/api/json/:id",
		)?.handlers[0];
	...
	it("should handle controllers returning Buffers, Uint8Arrays, and custom primitives", async () => {
		...
		expect(next).not.toHaveBeenCalled();
		expect(
			(dummyRes as unknown as { send: ReturnType<typeof vi.fn> }).send,
