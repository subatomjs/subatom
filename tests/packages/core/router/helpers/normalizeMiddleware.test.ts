...
import type { IContextMiddleware, ILegacyHandler } from "../../../../../packages/core/router/types/router.types.js";
...
describe("normalizeMiddlewareToHandler", () => {
  ...
  it("should execute middleware with _fileConfig property as legacy handler", async () => {
    ...
    const handler = normalizeMiddlewareToHandler(fileMw as unknown as ILegacyHandler);
    await handler(req, res, next);
