...
describe("Response", () => {
	...
	describe("Body, JSON, and HTML Rendering", () => {
		...
		it("should serialize json bodies fluently", () => {
			...
			expect(raw.end).toHaveBeenCalledWith(
				JSON.stringify({ framework: "subatom" }),
			);
		});
	...
	describe("Streaming and File Methods", () => {
		it("should delegate write and end to stream methods", () => {
			...
			expect(streamMethods.resWrite).toHaveBeenCalledWith(
				raw,
				"chunk",
				"utf-8",
				cb,
			...
			response.end("final");
			expect(streamMethods.resEnd).toHaveBeenCalledWith(
				raw,
				"final",
				undefined,
				undefined,
		...
		it("should delegate stream and sendStream", async () => {
			...
			expect(streamMethods.resStream).toHaveBeenCalledWith(
				raw,
				mockReadable,
				undefined,
			);
			...
			expect(streamMethods.resSendStream).toHaveBeenCalledWith(
				raw,
				mockReadable,
				undefined,
			);
		...
		it("should delegate sendFile, download, and attachment", () => {
			...
			expect(streamMethods.resSendFile).toHaveBeenCalledWith(
				raw,
				"/var/data.txt",
				{},
			);
