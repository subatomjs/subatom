...
env.number = function number(key: string, defaultValue?: number): number | undefined {
  const raw = process.env[key];
  if (raw === undefined || raw === '') return defaultValue;

  const parsed = Number(raw);
  if (Number.isNaN(parsed)) {
    throw new EnvError(`Environment variable "${key}" is not a valid number: "${raw}"`);
  }
  return parsed;
};

/** Read an env var as a boolean. Accepts true/1/yes/on (case-insensitive). */
env.bool = function bool(key: string, defaultValue?: boolean): boolean | undefined {
  const raw = process.env[key];
  if (raw === undefined || raw === '') return defaultValue;
...
env.array = function array(key: string, separator = ',', defaultValue: string[] = []): string[] {
  const raw = process.env[key];
  if (raw === undefined || raw === '') return defaultValue;
  return raw
    .split(separator)
...
env.json = function json<T = unknown>(key: string, defaultValue?: T): T | undefined {
  const raw = process.env[key];
  if (raw === undefined || raw === '') return defaultValue;
  try {
