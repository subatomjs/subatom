...
import { KeyResolver } from "./rateLimit.config.js";

export async function resolveKey(req: any, resolver: KeyResolver): Promise<string> {
  ...
  switch (resolver) {
    ...
    case "api-key":
      ...
      return apiKey ? `apikey:${hashSecret(String(apiKey))}` : resolveKey(req, "ip");
    case "tenant":
