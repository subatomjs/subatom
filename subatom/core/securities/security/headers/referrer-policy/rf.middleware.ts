import { ReferrerPolicyConfig, defaultReferrerPolicyConfig } from './rf.config.js'
...
export function createReferrerPolicyMiddleware(options?: Partial<ReferrerPolicyConfig> | boolean) {
  const config = normalizeConfig(defaultReferrerPolicyConfig, options);
  if (!config) return (_req: any, _res: any, next: () => void) => next();

  const value = Array.isArray(config.policy) ? config.policy.join(', ') : config.policy;
