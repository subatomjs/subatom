...
import { IWebSocketHandlers, IWebSocketOptions, IWebSocketRoute } from "../../types/framework/websocket/IWebSocket.js";
	...
	public activate(userOptions: IWebSocketOptions = {}): void {
		...
		this.httpServer.on("upgrade", (request, socket, head) => {
			...
			handleUpgrade(
				...
				this.routes,
				this.options,
				(rawSocket, req, route) => this.onConnectionEstablished(rawSocket, req, route),
			).catch((err) => {
...
// WebSocketManager.ts (updated methods)
    ...
    private onConnectionEstablished(
        ...
        const connection = new WebSocketConnection(rawSocket, request, this.registry, maxMsgs);
        ...
        rawSocket.on("close", (code, reasonBuf) => {
            ...
            try {
                ...
                console.error("[Subatom WS] onClose handler threw:", (err as Error).message);
            }
        });
        ...
        rawSocket.on("error", (err) => {
            ...
            try {
                ...
                console.error("[Subatom WS] onError handler threw:", (handlerErr as Error).message);
            }
        });
        ...
        try {
            ...
            console.error("[Subatom WS] onConnection handler threw:", (err as Error).message);
        }
    }
	...
	public async shutdown(timeoutMs?: number): Promise<void> {
		...
		await shutdownConnections(
			this.registry,
			timeoutMs ?? this.options.shutdownTimeoutMs ?? DEFAULT_OPTIONS.shutdownTimeoutMs,
		);
		this.wss?.close();
		this.active = false;
	}
