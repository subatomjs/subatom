...
export function resolveEntry(configuredEntry: string, cwd: string = process.cwd()): string {
  ...
  for (const candidate of candidates) {
    ...
    if (existsSync(abs)) {
      if (candidate !== configuredEntry) {
        logger.warn(`Configured entry not found, using detected entry: ${candidate}`);
      }
      return abs;
    }
  }
