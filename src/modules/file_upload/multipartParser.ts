...
import type { FileParserConfig } from "../../types/file_upload/TypeUploadFile.js";
	...
	return new Promise((resolve, reject) => {
		...
		if (config.storage === "disk" && config.dest) {
			...
			try {
				...
				return reject(
					new BadRequestError(
						`Failed to create upload directory '${targetDir}': ${mkdirErr.message}`,
					),
				);
			}
		}
		...
		try {
			...
			return reject(
				new BadRequestError(`Invalid multipart form headers: ${err.message}`),
			);
		}

		const body: Record<string, unknown> = {};
		const files: Record<string, UploadFile[]> = Object.create(null);
		const pendingWrites: Promise<UploadFile | null>[] = [];
		const tempFilePaths: string[] = []; // Track created temp files
		// NEW: track every open write/read handle so we can force-close them
		...
		const abortParsing = (err: Error) => {
			...
			// NEW: force-close every in-flight file stream / write stream.
		...
		bb.on("file", (fieldname, fileStream, info) => {
			// NEW: if we've already aborted (e.g. an earlier part in this same
			...
			if (config.storage === "memory") {
				...
				const promise = new Promise<UploadFile | null>((res, rej) => {
		...
		// NEW: if the underlying request itself dies mid-upload (client
...
//         return abortParsing(
//           new UnprocessableEntityError(
//             `File MIME type '${mimeType}' is not allowed for field '${fieldname}'`
//           )
//         );
//       }
