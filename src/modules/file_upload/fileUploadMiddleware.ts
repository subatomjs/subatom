...
import type { UploadFile } from "./UploadFile.js";
...
export function single(
	fieldname: string,
	options: FileMiddlewareOptions = { storage: "memory" },
): Middleware {
	return async (req: FrameworkRequest, res: any, next: (err?: any) => void) => {
...
export function fields(
	...
	return async (req: FrameworkRequest, res: any, next: (err?: any) => void) => {
		...
		try {
			...
			const resultFiles: Record<string, UploadFile[]> = Object.create(null);
...
async function cleanupFiles(
	filesMap: Record<string, UploadFile[]>,
): Promise<void> {
