  ...
  if (args.length === 1 && isRouteOptions(args[0])) {
    ...
    if (currentContext) {
      ...
      const combinedMiddlewares = [
        ...(currentContext.rateLimitMiddleware ? [currentContext.rateLimitMiddleware] : []),
        ...currentContext.middlewares,
        ...(opts.middleware || []),
      ];
      const mergedTags = Array.from(new Set([...currentContext.tags, ...(opts.tags || [])]));
      const effectiveRateLimit = opts.rateLimit ?? currentContext.rateLimitSpec;

      targetRouter.registerWithMeta(
        method,
        fullPath,
        [...(opts.middleware || []) as any, opts.controller as any],
        {
          name: opts.name,
