// src/rate-limit/stores/memory.store.ts
...
export class MemoryStore implements RateLimitStore {
  ...
  async evaluate(p: StoreEvalParams) {
    const entry = this.cache.get(p.key) || { count: 0, resetAt: p.now + p.windowMs, tokens: p.capacity, lastRefill: p.now };
    ...
    if (p.algorithm === "sliding-window") {
      const history: number[] = (entry.history || []).filter((ts: number) => ts > p.now - p.windowMs);
      history.push(p.now);
  ...
  private cleanup() {
    ...
    for (const [key, val] of this.cache.entries()) {
      if (val.resetAt && now > val.resetAt + 10000) this.cache.delete(key);
    }
  }

  async close() { clearInterval(this.gcInterval); }
}