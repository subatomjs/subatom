import { FrameguardConfig, defaultFrameguardConfig } from './frameguard.config.js';
...
export function createFrameguardMiddleware(options?: Partial<FrameguardConfig> | boolean) {
  ...
  return (_req: any, res: any, next: () => void) => {
    setHeader(res, SECURITY_HEADERS.X_FRAME_OPTIONS, config.action || 'SAMEORIGIN');
    next();
  };
}