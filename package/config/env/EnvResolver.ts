// src/config/env/EnvResolver.ts
import { configEnv, env } from "./env.js"; // Existing env module
import type { SubatomUserConfig } from "../../types/config/SubatomConfig.js";
 ...
 */
export function getEnvConfigOverride(): SubatomUserConfig {
    const override: SubatomUserConfig = {};
    ...
    if (env.has("SUBATOM_SOURCEMAP")) override.sourcemap = env.getBoolean("SUBATOM_SOURCEMAP");
    if (env.has("SUBATOM_MINIFY")) override.minify = env.getBoolean("SUBATOM_MINIFY");
    if (env.has("SUBATOM_WEBSOCKET")) override.websocket = env.getBoolean("SUBATOM_WEBSOCKET");
