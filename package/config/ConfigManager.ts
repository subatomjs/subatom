...
import { resolveEnvironmentFiles, getEnvConfigOverride } from "./env/EnvResolver.js";
import { validateConfig } from "./ConfigValidator.js";
import type { SubatomConfig, SubatomUserConfig } from "../types/config/SubatomConfig.js";
...
export class ConfigManager {
     ...
     */
    public static async resolve(runtimeOverrides?: SubatomUserConfig, forceReload = false): Promise<Readonly<SubatomConfig>> {
    ...
    public static get(): Readonly<SubatomConfig> {
        if (!cachedConfig) {
            throw new Error("[Subatom] Configuration has not been resolved yet. Call ConfigManager.resolve() first.");
        }
        return cachedConfig;
    }
}