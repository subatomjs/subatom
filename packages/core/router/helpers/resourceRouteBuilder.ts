/**
 * @fileoverview Builds RESTful resource routes from controller actions, supporting CRUD mappings, middleware,
 * naming, params, only/except, PATCH aliases, and metadata.
...
function normalizeActionHandlers(
	...
	return rawList.map((item, index) => {
		...
		return normalizeHandler(item as unknown as IController, isController);
	});
}
...
export function buildResourceRoutes(
	...
	const sharedMiddleware = (options.middleware ?? []).map((mw) =>
		normalizeHandler(mw as unknown as IController, false),
	);
	...
	for (const action of actions) {
		...
		const ownHandlers = normalizeActionHandlers(
			implementation,
			basePath,
			action,
		);
